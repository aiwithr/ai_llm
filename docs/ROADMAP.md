...
|> Last updated: 2026-06-06 — A5 complete (commit pending); rules: `site_url` = live site, `repo_url` = canonical project repo, project name = **AI Work Flow for Business**
...
| 2026-06-06 | A5 (Option B — surgical) — orphan links closed | 3 stale "Next steps" links in `from-rules-to-ai.md` rewritten to the live siblings: `rag-qwen.md` → `qwen-rag/index.md`; `reasoning-importance.md` → `ai-development/reasoning.md`; `enterprise-apps.md` → `enterprise-apps/index.md`. New "Where this fits" block added with 4 cross-links into `adoption/`, `architecture/`, and `gemma-e4b/slm-apps.md`. Build drops from 7 to 4 forward-looking warnings (3 from-rules-to-ai orphans closed; remaining 4 are 3 case-studies placeholders for A6 + 1 `reference/python-api.md` for A7). No folder moves, no mkdocs.yml edits, no cross-reference churn — the 7 module pages and the 7+ Bangla mirrors are untouched. |
## 9. Open questions
