...
|| 2026-06-17 | A8 partial — strict build green + Bangla nav expansion (single commit) | Two-pass fix. **Pass 1 — strict-build warning sweep.** The 2026-06-13 row claimed "0 warnings" but the real strict-build baseline on 2026-06-17 was **4 `WARNING -` lines**: 3 sibling-link orphans in English `from-rules-to-ai.md` (`rag-qwen.md`, `reasoning-importance.md`, `enterprise-apps.md` — last one had been carried over from the 2026-06-07 A5 row, which only closed 3 of 4 orphans) + 1 parent-link bug in `bangla/adoption/scale.md` (`../ROADMAP.md` → `../../ROADMAP.md`). Fixes: rewrote the 3 sibling links in `from-rules-to-ai.md` to their actual paths (`qwen-rag/index.md`, `ai-development/reasoning.md`, `enterprise-apps/index.md`); fixed the scale.md path to `../../ROADMAP.md`. **Pass 2 — Bangla nav expansion.** Three `mkdocs.yml` subsections that listed only `index.md` were expanded to surface their on-disk child pages: `বাংলা - আর্কিটেকচার` (+3 children: layers, data-flow, security → 4 entries), `বাংলা - কেস স্টাডিজ` (+3: isp-support, bank-it, factory-it → 4 entries), `বাংলা - রেফারেন্স` (+6: python-api, cli, prompts, benchmarks, glossary, conventions → 7 entries). **12 new nav entries** total. `বাংলা - AI ডেভেলপমেন্ট` remains index-only (the on-disk `index.md` was deleted 2026-06-10 for mojibake; the 3 child pages `citizen-developers.md`/`reasoning.md`/`sdlc.md` exist on disk but A8 mirror content for that section is still pending re-translation, not nav wiring). Build drops to **0 real `WARNING -` lines** (only the pre-existing Material for MkDocs 2.0 deprecation notice remains, which §6 explicitly excludes). AUDIT.md got a new `## 14. A8 follow-up` section. A8 mirror *content* (not just nav) is still the next open work item. |
| 2026-06-17 | A9 — Forward Deployed Engineering chapter (single commit) | One new English chapter `docs/adoption/fde.md` (~250 lines) + Bengali mirror `docs/bangla/adoption/fde.md` (~300 lines). 2 new nav entries. 4 cross-link edits: `adoption/index.md` 5th phase-table row + new "FDE lens" section; `discover.md` Audit-phase callout; `bangla/adoption/index.md` 5th table row + Bengali FDE-lens section; `reference/glossary.md` new `FDE` entry under `## F` (alphabetically before "Frozen eval set"). The chapter names the role the book already teaches (the senior engineer embedded with the customer who ships custom AI on the customer's own hardware and hands it over) and gives the four phases an industry label. FDE methodology is **Audit → Evals → Deployment**, mapping 1:1 to **Discover → Pilot → Build + Scale** — that 1:1 map is why the chapter belongs in `adoption/` and not in `reference/`. AUDIT.md got a new `## 15. A9` section. Build remains at **0 real `WARNING -` lines** (the cross-links to `fde.md` from `adoption/index.md`, `discover.md`, and `bangla/adoption/index.md` would have surfaced any orphan target at build time; all resolve). Open work: the English `fde.md` still has no "কেস স্টাডিজ" anchor — the case studies link forward to themselves, not into the FDE chapter; this is a small follow-up if the FDE lens is later extended per case study. |
| 2026-06-18 | বাংলা - রেফারেন্স nav label transliteration refinement | User-edited `mkdocs.yml` (lines 194-200) to refine 5 Bengali labels in the `বাংলা - রেফারেন্স` subsection to more standard Roman-to-Bengali spellings: `গুলোরাভিউ` to `গুলোরিভিউ` (more standard -ওভার- form); `বেনচমার্ক` to `বেঞ্চমার্ক` (chandrabindu ঞ + র, matches the spoken form better); `গলসারি` to `গ্লসারি` (chandrabindu on গ্ল); `কনভেনশন` and `প্রম্পট` kept as-is (already canonical). Labels are display strings only, no path changes, no audit-log link breakage. Strict build re-verified: exit 0, 0 `WARNING -` lines. No AUDIT.md new section needed (transliteration is a label-only refinement, not a deliverable); recorded here in the decision log so a future contributor does not "correct" the spellings back to the less-standard forms. |
| 2026-06-18 | Landing page simplification (A10 - bilingual English/বাংলা tables, FDE chapter surfaced) | Rewrote `docs/index.md` as a shorter, easier first-page read. New hero ("Adopt AI in your business. Keep your data on your network.") + a "One sentence, two languages" tip callout + a 4-row "The core idea" bilingual table. New "How a Forward Deployed Engineer helps your business" section with a 3-row Audit/Evals/Deployment table (English + বাংলা columns) + a "Why an FDE is mandatory for local-LLM businesses" info callout, surfacing the A9 chapter directly on the landing page. "Start here" grid now has 5 cards: Why, See it run, **FDE** (new), Adoption journey, Architecture. "Who this is for" table now has 3 columns: Audience, English, বাংলা. The local-gateway mermaid diagram is simpler (flat `flowchart LR`, no `subgraph` wrapper) and the 5-step explainer is gone - the diagram is enough for a first-page read. The "What's in the box" and "Project plan" sections are removed (the grid + audience table cover their job). The Bangla contact callout (`!!! question "কথা বলতে চাচ্ছেন?"` with WhatsApp/Medium/Facebook/LinkedIn/YouTube) is preserved verbatim at the very bottom per user request. Strict build re-verified: exit 0, 0 `WARNING -` lines (the 7 pre-existing `bangla/reference/*` anchor INFO lines are unchanged from before the rewrite; no new warnings on the English landing page or its FDE link). No AUDIT.md new section needed (landing-page iteration on existing A2 deliverable, not a new deliverable). `docs/bangla/index.md` is **not** mirrored in this commit - the user's "both in Bangla and English" was interpreted as "make the *English* landing page readable in both languages simultaneously," and the Bangla mirror remains its own page (a follow-up commit can revisit the Bangla landing page if the user wants the same surface treatment there). |
| 2026-06-19 | Landing page accessibility pass (A11 - language toggle, FDE in hero, slim audience table) | Three small additions to `docs/index.md` (single commit, ~10 lines net). (1) **Language toggle** — a new `!!! info "Read this page in your language"` callout below the hero, pointing readers who want the full site in বাংলা to `bangla/index.md` (the existing Bangla mirror, no changes to it). (2) **FDE in hero** — added a second blockquote line to the hero: "A Forward Deployed Engineer (FDE) makes the rollout real — they audit your data, build evals, and ship the gateway. Read the full FDE chapter." with a link to `adoption/fde.md`. This answers the "should I even read this?" question a first-time visitor has, without requiring a scroll past the bilingual tables. (3) **Slim audience table** — removed the 4th "University IT (later phase)" row from the "Who this is for" table and replaced it with a one-line forward pointer: "University IT, government, and other sectors — see Case Studies for the full audience map." (the case-studies section already covers those audiences honestly). All three changes target Bangladeshi readers who know both English and বাংলা: the language toggle surfaces the full-bangla path; the hero FDE line answers the role question in plain English; the slimmer table removes noise about a phase that isn't here yet. The "Who this is for" table goes from 4 rows to 3. The Bangla contact callout at the bottom is untouched. `docs/bangla/index.md`, `docs/adoption/fde.md`, `mkdocs.yml`, and the `## 9. Open questions` section are all unchanged. Strict build re-verified: exit 0, 0 `WARNING -` lines (the 7 pre-existing `bangla/reference/*` anchor INFO lines are unchanged from the A10 baseline). No AUDIT.md new section needed (this is a follow-up landing-page iteration, not a new deliverable). |
## 9. Open questions
